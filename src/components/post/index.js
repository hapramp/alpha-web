...
class Post extends React.Component {
  ...
  render() {
    ...
    const content = this.props.post.json_metadata.content;

    /* Author details */
    const user = this.props.postingUser;

    /* Render */
    return (<div className={['uk-margin-bottom', this.props.border ? styles.postContainerBorder : '', indexStyles.white].join(' ')}>
      {/* Top section */}
      <PostUserMeta
        profile={{ name: user.json_metadata.profile.name, image: user.json_metadata.profile.profile_image, username: user.name }}
