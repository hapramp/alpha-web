  ...
  handlePostCreate() {
    ...
    if (this.props.media) {
      if (this.props.media.type === 'image') {
        ...
        const uploadTask = window.firebaseStorage.ref().child(`images/${new Date().toISOString()}${this.props.media.content.name}`)
    ...
    return (<div className={['uk-margin-top'].join(' ')}>
      ...
      <div uk-grid="true" className="uk-margin-remove">
        {this.props.communities.map(community =>
          (<div
            key={community.id}
            onClick={() => this.props.changeCommunity(community.id)}
            className={[styles.communitySingle,
								 _.some(this.props.activeCommunity, i => i === community.id) ? styles.active : ''].join(' ')}
          >{community.name}
           </div>))}
      </div>
            </div>);
  ...
  showHashtags() {
    return (<div uk-grid="true" className={['uk-margin-remove'].join(' ')}>
      {this.props.hashtags.length ?
				this.props.hashtags.map(i => (<span
  key={i}
